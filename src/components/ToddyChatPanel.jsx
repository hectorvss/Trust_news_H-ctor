  ...
  useEffect(() => {
    ...
    const loadConversation = async () => {
      ...
      try {
        ...
        if (!res.ok) throw new Error(payload.error || 'No se pudo cargar Toddy');
  ...
  const sendMessage = async (text = message) => {
    ...
    try {
      ...
      if (!res.ok || !res.body) {
        ...
        if (payload.error === 'free_limit_used') throw new Error('Ya usaste la pregunta gratuita de Toddy para esta noticia.');
        if (payload.error === 'insufficient_credits') throw new Error('No tienes creditos IA suficientes para esta profundidad.');
        throw new Error(payload.error || 'Toddy no pudo responder');
      }
        ...
        {!user ? (
          ...
          <>
            ...
            <form
              ...
              style={{ borderTop: '1px solid #111', padding: '14px', background: '#f7f5ef', display: 'flex', gap: '10px' }}
            >
              <input
                value={message}
                ...
                style={{ flex: 1, minWidth: 0, border: '1px solid #111', background: '#fff', padding: '12px', fontSize: '14px', outline: 'none' }}
              />
              <button
                disabled={!message.trim() || !canAsk || needsCredits || freeBlocked}
                style={{ border: '1px solid #111', background: '#111', color: '#fff', padding: '0 16px', fontWeight: 900, cursor: 'pointer', opacity: (!message.trim() || !canAsk || needsCredits || freeBlocked) ? 0.45 : 1 }}
              >
                ENVIAR
              </button>
            </form>
