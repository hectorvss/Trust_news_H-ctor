...
const Discover = ({ navigate, setActiveCategory }) => {
  ...
  const goTopic = (t) => { if (setActiveCategory && TOPICS.includes(t)) setActiveCategory(t); navigate(`/?topic=${encodeURIComponent(t)}`); };
  const goPlace = (p) => navigate(`/?city=${encodeURIComponent(p)}`);
  ...
  const Row = ({ icon, label, sublabel, followKey, onOpen }) => {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 0', borderBottom: '1px solid #eee' }}>
        <div onClick={onOpen} style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1, cursor: onOpen ? 'pointer' : 'default', minWidth: 0 }}>
          {icon}
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: '14px', fontWeight: 700, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{label}</div>
        ...
        <button
          ...
          style={{
            ...
            border: '1px solid #000', background: following ? '#000' : '#fff', color: following ? '#fff' : '#000',
            fontWeight: 900, fontSize: '15px', lineHeight: 1, display: 'flex', alignItems: 'center', justifyContent: 'center'
          }}
        >
          {following ? '✓' : '+'}
        </button>
  ...
  const monogram = (text) => (
    <div style={{ width: 32, height: 32, borderRadius: '50%', background: '#000', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontFamily: 'var(--font-mono)', fontSize: '13px', flexShrink: 0 }}>
  ...
  const Section = ({ title, items, renderRow, sectionKey, searching }) => {
    ...
    return (
      <section style={{ marginBottom: '56px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '20px', borderBottom: '2px solid black', paddingBottom: '12px' }}>
          <h2 style={{ fontSize: isMobile ? '28px' : '36px', fontWeight: 800, letterSpacing: '-1.5px', margin: 0 }}>{title}</h2>
          {items.length > (isMobile ? 5 : 8) && (
            <span onClick={() => setExpanded(p => ({ ...p, [sectionKey]: !isOpen }))} style={{ fontSize: '11px', fontWeight: 900, fontFamily: 'var(--font-mono)', cursor: 'pointer', letterSpacing: '1px' }}>
  ...
  return (
    <div style={{ maxWidth: 'var(--content-width)', margin: '0 auto', padding: isMobile ? '40px 16px' : '60px var(--page-padding)' }}>
      ...
      <div style={{ marginBottom: '40px' }}>
        <div style={{ fontSize: '11px', fontWeight: 900, fontFamily: 'var(--font-mono)', letterSpacing: '2px', opacity: 0.3, marginBottom: '16px' }}>TNE / DESCUBRIR</div>
      ...
      {/* Sub-tabs (visual; functional where it makes sense) */}
      ...
      <Section
        ...
        renderRow={(s) => (
          <Row
            key={s.id}
            icon={<SourceLogo source={s} size={32} />}
            label={s.name}
            sublabel={<SourceTag kind="bias" value={s.biasRating} />}
            followKey={`source:${s.id}`}
            onOpen={() => navigate(`/?topic=${encodeURIComponent(s.name)}`)}
          />
        )}
      />
