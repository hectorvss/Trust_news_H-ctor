...
const SpecialSection = ({ idx, section, stories, onSelectStory, navigate }) => {
  ...
  // Estos bloques son editoriales: pueden traer un story_id (a veces fantasma)
  // o ninguno. Resolvemos por id → por texto → búsqueda, para que SIEMPRE
  // lleven a su noticia como un click normal.
  const openItem = (storyId, text) => {
    if (storyId) {
      const story = stories.find(s => String(s.id) === String(storyId));
      if (story) { onSelectStory(story); return; }
    }
    const match = findStoryByText(text, stories);
    if (match) { onSelectStory(match); return; }
    if (text) navigate(`/search?q=${encodeURIComponent(String(text).slice(0, 80))}`);
  };

  const handleMainClick = () => openItem(main.story_id, main.title || main.desc);
  const handleSideClick = (side) => openItem(side.story_id, side.title || side.label);
    ...
    <section key={section.id} className="layout-split" style={{ borderTop: idx === 0 ? 'var(--border-thin)' : '1px solid black', background: '#fff' }}>
      <div className="sidebar" style={{ display: 'flex', flexDirection: 'column', padding: '60px 40px', borderRight: 'var(--border-thin)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <button onClick={handleMainClick} style={{ padding: '14px', background: 'black', color: 'white', border: 'none', borderRadius: '4px', fontSize: '10px', fontWeight: 900, cursor: 'pointer', letterSpacing: '1px' }}>
      ...
      <div className="main-content" style={{ padding: 0 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', minHeight: '600px' }}>
          ...
          <div
            ...
            style={{ padding: '60px', borderRight: 'var(--border-thin)', display: 'flex', flexDirection: 'column', gap: '24px', cursor: 'pointer', transition: 'background 0.15s' }}
